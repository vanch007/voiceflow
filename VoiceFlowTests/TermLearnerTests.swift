...
final class TermLearnerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        let clearExpectation = expectation(description: "Clear all data")
    ...
    func testTermDeduplication() {
        ...
        let analyzeExpectation = expectation(description: "Analyze frequency")
        var analyzeFulfilled = false

        termLearner.onSuggestionsChanged = {
            if !analyzeFulfilled {
                analyzeFulfilled = true
                analyzeExpectation.fulfill()
            }
        }
    ...
    func testExportFormatWithMetadata() {
        ...
        // Add auto-learned term
        let autoTerm = LearnedTerm(
            term: "MLX",
            frequency: 5,
            source: .autoLearned,
            isApproved: true
    ...
    func testImportMergeWithoutDuplicates() {
        ...
        let analyzeExpectation = expectation(description: "Analyze frequency")
        var analyzeFullfilled = false

        termLearner.onSuggestionsChanged = {
            if !analyzeFullfilled {
                analyzeFullfilled = true
                analyzeExpectation.fulfill()
            }
        }
