...
⚠️ **중요: VoiceFlow 플러그인 시스템은 TRUST-BASED 보안 모델을 사용합니다**

플러그인은 **샌드박스 없이** VoiceFlow 애플리케이션과 동일한 프로세스에서 실행됩니다. 이는 다음을 의미합니다:

- ❌ **파일시스템 격리 없음**: 플러그인은 사용자가 접근 가능한 모든 파일을 읽고, 수정하고, 삭제할 수 있습니다
- ❌ **프로세스 격리 없음**: 악의적이거나 버그가 있는 플러그인이 전체 애플리케이션을 크래시시킬 수 있습니다
- ❌ **메모리 격리 없음**: 플러그인이 메모리 누수를 일으켜도 제한이 없습니다
- ❌ **네트워크 격리 없음**: 플러그인은 제한 없이 네트워크 요청을 할 수 있습니다
- ⚠️ **권한 시스템**: manifest의 permissions는 **문서화 목적**이며 기술적으로 강제되지 않습니다

**사용자를 위한 보안 권장사항:**
1. ✅ **신뢰할 수 있는 소스의 플러그인만 설치**
2. ✅ **가능하면 플러그인 소스 코드 검토**
3. ✅ **manifest 권한 확인** - 단순한 텍스트 변환 플러그인이 `network.http`를 요청하면 의심
4. ✅ **새 플러그인 설치 후 시스템 리소스 모니터링**
5. ✅ **사용하지 않는 플러그인 제거**

**플러그인 개발자를 위한 보안 모범 사례:**

1. **권한 최소화**: 실제로 필요한 권한만 manifest에 선언
2. **입력 검증**: 모든 텍스트 입력 검증 및 sanitize
3. **에러 처리**: 예외를 적절히 처리하여 호스트 애플리케이션 크래시 방지
   ```python
   # ✅ Good
   try:
       process_text(text)
   ...
   # ❌ Bad
   sys.exit(1)  # Crashes entire app
   ```
4. **메모리 관리**: `onUnload()`에서 모든 리소스를 정리하여 메모리 누수 방지
   ```python
   async def on_unload(self):
       self.cache.clear()  # Clean up
       self.connections.close()  # Close connections
   ```
5. **네트워크 보안**:
   - HTTPS 사용
   - 자격증명을 안전하게 저장 (macOS Keychain, 환경 변수)
   - 사용자 데이터를 외부 서버로 전송하기 전에 동의 획득
6. **비동기 작업**: 블로킹 작업 피하기 (async/await 사용)
7. **코드 공개**: 가능하면 오픈 소스로 공개하여 보안 검토 허용
8. **문서화**: 플러그인의 보안 영향 명시

**금지 사항:**
- ❌ 사용자 동의 없이 외부 서버로 데이터 전송
- ❌ 의도적으로 애플리케이션 크래시 (`sys.exit()`, `fatalError()`)
- ❌ 플러그인 디렉토리 외부의 파일 무단 수정
- ❌ 악의적인 코드 실행 또는 시스템 손상
